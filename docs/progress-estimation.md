...
| Crawler ingestion | 84% | strong | Per-office adapters, checksum validation, DB-backed ingest, change log persistence, crawler run tracking, and notification job creation are wired. |
...
| Deployability | 84% | strong | Vercel/Supabase/Koyeb plan, Terraform scaffold, CI, deploy-smoke workflow, `readyz`, and smoke scripts are in place; real secret provisioning and first live apply are still pending. |
...
- Deployment assets are present, but a full live environment run has not been executed from this repo yet.
...
5. Apply the stack to a real environment and run `.github/workflows/deploy-smoke.yml`.

## Confidence Notes

- Confidence in the `90%` feature-progress estimate: medium
- Confidence in the `74%` production-readiness estimate: high
